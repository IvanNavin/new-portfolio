      ...
      theory: [
        {
          kind: 'text',
          text:
            'Access-лог nginx — це по рядку на запит: IP, час, метод і шлях, ' +
            'код відповіді, розмір і **тривалість**. Останні дві колонки часто важливіші за все інше.',
        },
        {
          kind: 'code',
        ...
        },
        {
          kind: 'note',
          text:
            'Зверни увагу на тривалість `30.011` — це рівно таймаут. Коли всі помилки ' +
            'мають однаковий час близько круглого числа, це майже завжди впертий таймаут ' +
            'до залежності, а не випадковий збій.',
        },
      ],
      task: {
        kind: 'terminal',
        ...
        goals: [
          ...
          },
          {
            ...
            check: (s) =>
              (readFile(s.fs, '/home/deploy/count-502.txt') ?? '').trim() ===
              '5',
          },
          {
            id: 'endpoint',
            ...
            check: (s) =>
              (readFile(s.fs, '/home/deploy/endpoint.txt') ?? '').trim() ===
              '/checkout',
          },
        ],
      },
...

    {
      ...
      theory: [
        ...
        },
        {
          kind: 'note',
          text:
            'Видаляти активний лог-файл через `rm` — погана ідея: процес тримає дескриптор, ' +
            'і місце не звільниться до перезапуску сервісу. Правильно — **обнулити** його ' +
            'через `> file.log`, файл лишається на місці.',
        },
      ],
      task: {
        kind: 'terminal',
      ...
      task: {
        ...
        goals: [
          ...
          },
          {
            ...
            check: (s) =>
              (readFile(s.fs, '/home/deploy/cause.txt') ?? '')
                .trim()
      ...
      theory: [
        {
          ...
          text:
            ...
            'Пошук винного, красиве рішення й рефакторинг — усе потім.',
        },
        {
          kind: 'note',
          text:
