      ...
      theory: [
        ...
        },
        {
          ...
          rows: [
            ...
            [
              'COPY src dst',
              'покласти файли з контексту збірки в образ. Копіюють обидва файли залежностей: package.json і package-lock.json',
            ],
        ...
        },
        {
          kind: 'text',
          text:
            'Команда, якою ставлять залежності Node, — `npm ci`. Вона читає ' +
            '`package-lock.json` і ставить рівно ті версії, що там зафіксовані, — ' +
            'тому збірка сьогодні й через рік дає той самий результат. ' +
            'Прапорець `--omit=dev` пропускає пакети, потрібні лише для розробки: ' +
            'у продакшн-образі їм робити нічого.',
        },
        {
          kind: 'note',
          text:
