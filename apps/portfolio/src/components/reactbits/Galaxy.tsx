...
export default function Galaxy({
  ...
  useEffect(() => {
    ...
    function resize() {
      // Layout size (offset*) is immune to the cube's 3D transforms, unlike
      // getBoundingClientRect, which is skewed on a rotated face.
    ...
    // Listen on window (not the container): the background sits behind
    // pointer-events:none content, so container-level events never fire.
  ...
  return (
    <div
      ref={ctnDom}
      className={`pointer-events-none relative h-full w-full ${className}`.trim()}
    />
  );
}
